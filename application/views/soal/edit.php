                    ...
                    <div class="col-sm-8 col-sm-offset-2">
                        <div class="row">
                            <div class="form-group col-sm-12">
                                <label for="guru_id" class="control-label">Guru (Pelajaran)</label>
                                <?php if( $this->ion_auth->is_admin() ) : ?>
                                <select required="required" name="guru_id" id="guru_id" class="select2 form-group" style="width:100% !important">
                                    <option value="" disabled selected>Pilih Guru</option>
                                    <?php 
                                    $sdm = $soal->guru_id.':'.$soal->pelajaran_id;
                                    foreach ($guru as $d) : 
                                        $dm = $d->id_guru.':'.$d->pelajaran_id;?>
                                        <option <?=$sdm===$dm?"selected":"";?> value="<?=$dm?>"><?=$d->nama_guru?> (<?=$d->nama_pelajaran?>)</option>
                                    <?php endforeach; ?>
                                </select>
                                <small class="help-block" style="color: #dc3545"><?=form_error('guru_id')?></small>
                                <?php else : ?>
                                <input type="hidden" name="guru_id" value="<?=$guru->id_guru;?>">
                                <input type="hidden" name="pelajaran_id" value="<?=$guru->pelajaran_id;?>">
                                <input type="text" readonly="readonly" class="form-control" value="<?=$guru->nama_guru; ?> (<?=$guru->nama_pelajaran; ?>)">
                                <?php endif; ?>
                            </div>
